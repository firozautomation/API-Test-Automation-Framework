...
Feature: Service Request Service Performance Test
    ...
    * url 'http://' + env + '-' + ch + '-shared-services-service-request-service.' + env + '.cloud.ds.gehc.net/serviceRequest/v1/'
    ...
    #*******************************************************************************************************************
  	...
  	Scenario: Insert X valid Records into SR Tasks Post Method
   	* call JavaMethods.setGlobalPerformanceTestData(serviceRequestCode,sourceSystemCode,taskCode,assetTblAssetId,country)

	* def ServiceRequestPerformanceTemplate = JavaMethods.createTestData('SR Tasks: Insert', 100, staticRand, 0, 0)
    ...
    And header Content-Type = 'application/json'
  	...
  	* header Authorization = 'Bearer ' + Token
  	...
  	#Mongo Check Point
  	* def loadUID = $.loadId
  	* assert JavaMethods.checkMongoCollection('Tasks Performance', loadUID, 'INSERTED') == true	
  	
  	
  	#Maint DB Check Point
  	#* def sqlQuery = 'SELECT * FROM Task WHERE code LIKE \'Task-PerformanceTestUsingKarate%\' AND ServicedBy = \'' + staticRand + '\';'
