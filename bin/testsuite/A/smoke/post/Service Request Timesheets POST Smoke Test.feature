...
Feature: Service Request Timesheets POST Method Verification 
  ...
  Background:
    ...
    * header Authorization = 'Bearer ' + Token
    And param view = 'full'
    And param q = 'serviceRequestCode:' + serviceRequestCode + ' AND taskCode:' + taskCode
    When method get
    Then status 200
    And match response $..timesheets[*].code contains 'Karate-Insert: ' + NewTimesheet
  	
  	
  	Scenario: Submit a new Service Request Timesheets POST with an invallid Task Code
